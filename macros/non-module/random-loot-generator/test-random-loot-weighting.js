// Generated by scripts/admin/build-random-loot-weighting-test.mjs
...
(async () => {
  ...
  const CONSUMABLE_UNDERREPRESENTED_BOOST = 2.35;
  const POTION_FIRST_SHOWUP_BOOST = 6.5;
  const POTION_UNDERREPRESENTED_BOOST = 2.1;
  const HEALING_FIRST_SHOWUP_BOOST = 4.4;
  const HEALING_UNDERREPRESENTED_BOOST = 2.2;
  const CONSUMABLE_HEALING_ROLL_CHANCES = {
    potion: 0.5,
    scroll: 0.34,
    wand: 0.4,
  };
  const PERMANENT_OVERREPRESENTED_DAMPING = 0.33;
  const PERMANENT_STREAK_DAMPING = 0.72;
  const HEALING_POTION_NAME_WEIGHT = 3.8;
  const HEALING_SCROLL_WAND_NAME_WEIGHT = 2.6;
  const CONSUMABLE_REPEAT_DAMPING = 0.93;
  const SAME_CONSUMABLE_TYPE_DAMPING = 0.86;
  const SAME_CONSUMABLE_SPELL_DAMPING = 0.38;
  const DESIRED_CONSUMABLE_SHARE = 0.55;
  const DESIRED_HEALING_SHARE_OF_CONSUMABLES = 0.35;
  ...
  function createConsumableBucketCandidate(bucket) {
    ...
    const healingShare = spellOptions.length > 0 ? healingCount / spellOptions.length : 0;
  ...
  function getCandidateCategoryWeight(candidate, categoryCounts = {}) {
    ...
    if (candidate?.isHealing && Object.prototype.hasOwnProperty.call(HEALING_CATEGORY_WEIGHTS, sourceType)) {
      ...
      const healingShare = clampNumber(Number(candidate?.healingShare) || 0, 0, 1);
      weight *= 1 + (healingShare * 2.4);
    }
  ...
  function getCandidateRepeatPenalty(candidate, selectedItems = [], options = {}) {
    ...
    const nonCurrencySelections = (selectedItems || []).filter((item) => normalizeText(item?.sourceType) !== "coinsgemsart");
    const permanentSelections = nonCurrencySelections.filter((item) => !isConsumableSourceType(item?.sourceType));
    const potionSelections = consumableSelections.filter((item) => normalizeText(item?.sourceType) === "potion");
    const healingSelections = consumableSelections.filter((item) => !!item?.isHealing);
    const sourceType = normalizeText(candidate?.sourceType || "");
    const isConsumableCandidate = isConsumableSourceType(sourceType);
    const isHealingCandidate = !!candidate?.isHealing;
    ...
    const crackFillThreshold = maximumBudget > 0
      ...
      : 0;
    const currentConsumableShare = nonCurrencySelections.length > 0
      ? consumableSelections.length / nonCurrencySelections.length
      : 0;
    const currentPermanentShare = nonCurrencySelections.length > 0
      ? permanentSelections.length / nonCurrencySelections.length
      : 0;
    const currentHealingShareOfConsumables = consumableSelections.length > 0
      ? healingSelections.length / consumableSelections.length
      : 0;
    ...
    if (isConsumableCandidate) {
      ...
      if (currentConsumableShare < DESIRED_CONSUMABLE_SHARE) {
        crackFillWeight *= CONSUMABLE_UNDERREPRESENTED_BOOST;
      }
      if (sourceType === "potion" && !potionSelections.length) {
        crackFillWeight *= POTION_FIRST_SHOWUP_BOOST;
      } else if (sourceType === "potion" && potionSelections.length < Math.max(1, Math.floor(consumableSelections.length / 2))) {
        crackFillWeight *= POTION_UNDERREPRESENTED_BOOST;
      }
      if (isHealingCandidate && !healingSelections.length) {
        crackFillWeight *= HEALING_FIRST_SHOWUP_BOOST;
      } else if (isHealingCandidate && currentHealingShareOfConsumables < DESIRED_HEALING_SHARE_OF_CONSUMABLES) {
        crackFillWeight *= HEALING_UNDERREPRESENTED_BOOST;
      }
    } else if (currentPermanentShare >= (1 - DESIRED_CONSUMABLE_SHARE)) {
      crackFillWeight *= PERMANENT_OVERREPRESENTED_DAMPING;
    ...
    const permanentStreakPenalty = !isConsumableCandidate && permanentSelections.length > consumableSelections.length
      ? Math.pow(PERMANENT_STREAK_DAMPING, permanentSelections.length - consumableSelections.length)
      : 1;

    return crackFillWeight
      * permanentStreakPenalty
  ...
  async function materializeConsumableBucketCandidate(bucketCandidate, options = {}) {
    ...
    const preferredHealingRoll = shouldPreferHealingConsumableSpell(bucketCandidate, options.selectedItems);
    const healingOptions = spellOptions.filter((option) => option.isHealing);
    const nonHealingOptions = spellOptions.filter((option) => !option.isHealing);
    const optionGroups = preferredHealingRoll
      ? [
        healingOptions.length ? healingOptions : spellOptions,
        nonHealingOptions,
      ]
      : [
        nonHealingOptions.length ? nonHealingOptions : spellOptions,
        healingOptions,
      ];

    for (const optionGroup of optionGroups) {
      const remainingOptions = [...(optionGroup || [])];
      if (!remainingOptions.length) continue;
  ...
  function shouldPreferHealingConsumableSpell(bucketCandidate, selectedItems = []) {
    ...
    const healingChance = clampNumber(Number(CONSUMABLE_HEALING_ROLL_CHANCES[sourceType]) || 0, 0, 1);
    if (!(healingChance > 0)) return false;

    const healingOptions = (bucketCandidate?.spellOptions || []).filter((option) => option?.isHealing);
    if (!healingOptions.length) return false;

    const consumableSelections = (selectedItems || []).filter((item) => isSpellConsumableCandidate(item));
    const healingSelections = consumableSelections.filter((item) => !!item?.isHealing);
    const currentHealingShare = consumableSelections.length > 0
      ? healingSelections.length / consumableSelections.length
      : 0;
    const adjustedChance = currentHealingShare < DESIRED_HEALING_SHARE_OF_CONSUMABLES
      ? Math.min(1, healingChance * HEALING_UNDERREPRESENTED_BOOST)
      : healingChance;
